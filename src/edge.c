...
int inet_pton_ex(int af, const char *src, void *dst)
{
...
const char *inet_ntop_ex(int af, const void *src, char *dst, socklen_t size)
{
  struct sockaddr_storage ss;
...
static int scan_address (char * ip_addr, size_t addr_size,
    ...
    if(end) {
        ...
        inet_ntop_ex(AF_INET, &bitlen, netmask, netmask_size);
    }
